...
_group-provider.name should be either `file` or `ldap` and the configuration of the chosen group provider should be appended to this
file_
...
File group provider resolve group memberships using a file on the coordinator.
...
Resolves user group memberships from an LDAP server for system access control. This allows access rules to be defined based on LDAP groups instead of individual users.
...
* - `ldap.ignore-referrals`        
  - Referrals allow finding entries across multiple LDAP servers. Ignore them to only search within 1 LDAP server
  - 
...
* - `ldap.use-group-filter`        
  - Whether to use search-based group resolution. Defaults to `true`. If `false`, Trino uses the attribute-based method.
  - 
:::
